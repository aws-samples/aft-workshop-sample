...
# Retrieve the run task name from parameter store
data "aws_ssm_parameter" "app_runtask" {
  name = "/aft/account-request/custom-fields/app_runtask"
}

# Retrieve the run task enforcement level from parameter store
data "aws_ssm_parameter" "app_runtask_enforcement" {
  name = "/aft/account-request/custom-fields/app_runtask_enforcement"
}
...
# Verify the run task id from data source
data "tfe_organization_run_task" "app_runtask" {
  name         = data.aws_ssm_parameter.app_runtask.value
  organization = data.tfe_organization.app_org.name
}
...
#  Assign run task to the Application workspace
resource "tfe_workspace_run_task" "app_workspace" {
  workspace_id      = resource.tfe_workspace.app_workspace.id
  task_id           = data.tfe_organization_run_task.app_runtask.id
  enforcement_level = data.aws_ssm_parameter.app_runtask_enforcement.value
}
